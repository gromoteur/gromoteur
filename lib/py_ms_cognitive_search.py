...
class PyMsCognitiveSearch(object):
    ...
    def search(self, limit=50, format='json', offset=0):
        ''' Returns the result list, and also the uri for next page (returned_list, next_uri) '''
        return self._search(limit, format, offset)
...
class QueryChecker():
    ...
    def check_web_params(query_dict, header_dict):
        ...
        if 'count' in list(query_dict.keys()):
        ...
        if 'safeSearch' in list(query_dict.keys()):
            ...
            if 'X-Search-ClientIP' in list(query_dict.keys()):
                input('You have specified both an X-Search-ClientIP header and safesearch setting\nplease note: header takes precedence')
